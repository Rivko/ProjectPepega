...
try:
    with open(config.HBK_NAME, "rb") as hbk_file:
        ...
        logger.info(f"Loaded {config.HBK_NAME}, file size = {hbk.size()} bytes, addresses = {hbk_size-1}")
except FileNotFoundError as e:
    logger.error(e)
    exit()

tablichka_rows = ["ID", "Name", "Type", "Address", "Value hex original", "Disassemble", "Extracted Value", "Value override", "Value Override Converted to HEX"]
    ...
    try:
        with open(config.LIB_NAME, "r+b") as f:
            ...
            for i in range(hbk_size-1):
                ...
                address_offset = start_offset + 80  # адрес лежит через 80 байт после имени
                address = read_hbk_by_offset(address_offset)
                length_offset = start_offset + 208  # размер лежит через 208 после имени
                length = int(read_hbk_by_offset(length_offset), 16)
                array_count_offset = start_offset + 336 #количество значений для массивов лежит через 336 байт после имени
                array_count = int(read_hbk_by_offset(array_count_offset), 16)
                start_offset = start_offset + 656  # дальше букмарки идут через каждые 656 байт
                if array_count > 1:
                    ...
                    logger.info(f"#{i}, {name=}, {address=}, {length=}, {default_value.hex().upper()=}, {arm_type=}")
                    filewriter.writerow([i+1, name, arm_type, address, default_value.hex().upper()])
    except FileNotFoundError as e:
        logger.error(e)
        exit()